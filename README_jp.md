...
1. **ダイスロール** — 2D6 ＋ `MOVE` バフ分のボーナス＝このターンの移動力。
...
**ゲーム終了条件**は次のいずれかです：
- ラウンド上限（`TurnManager.MAX_ROUNDS = 15`）を超えた — この時点でまだ `ACTIVE`（潜行中）の
...
- **`CPUAI`** は状態を持たない単純なヒューリスティックです。前進先が無い場合、ライトが少ない
  ...
  空マスは常に無視、イベントはHP/ライト/スコアの増減からスコアリングして選択）。

## マップの作り方（`data/maps/*.txt`）

`MapTextLoader.gd` が解析します。マップファイルは**マス行**と**接続行**を交互に並べたもので、
